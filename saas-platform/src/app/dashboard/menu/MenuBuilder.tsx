    ...
    <>
      <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white border-r border-[#e2e8f0]">
          {/* Header */}
          <div className="px-4 md:px-6 py-4 md:py-5 border-b border-[#e2e8f0] flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
            ...
            {loading ? (
              ...
              return (
                <div key={cat.id} className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
                  {/* Category Header */}
                  <div className="group flex items-center gap-2 md:gap-3 bg-[#f9f9ff] p-2 md:p-3 border-b border-[#e2e8f0]">
                    <div className="text-[#c4c6cc] hover:text-[#0d1b2a] cursor-grab active:cursor-grabbing px-1 shrink-0">
                      <DotsSixVertical size={20} weight="bold" />
                    </div>
                    
                    <button
                      onClick={() => setCollapsed((p) => ({ ...p, [cat.id]: !p[cat.id] }))}
                      className="text-[#415a77] hover:text-[#0d1b2a] transition shrink-0"
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      {editingCatId === cat.id ? (
                        <input
                          ...
                          className="bg-white px-2 py-1 border border-[#0d1b2a] rounded text-sm font-semibold outline-none w-full max-w-[200px]"
                        />
                      ) : (
                        <h2 className="text-sm font-semibold text-[#0d1b2a] truncate">{cat.name}</h2>
                      )}
                    </div>

                    {/* Category Actions */}
                    <div className="opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity flex items-center gap-0.5 md:gap-1 shrink-0">
                      <button
                        onClick={() => setEditingCatId(cat.id)}
                        className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center text-[#74777d] hover:bg-[#e2e8f0] rounded transition"
                        title="Edit name"
                      >
                      ...
                      <button
                        ...
                        className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center text-[#74777d] hover:text-red-600 hover:bg-red-50 rounded transition"
                    ...
                    <button
                      ...
                      className="ml-1 md:ml-2 flex items-center justify-center gap-1.5 h-7 md:h-8 px-2 md:px-3 text-xs font-semibold text-[#0d1b2a] bg-white border border-[#e2e8f0] rounded-lg hover:border-[#c4c6cc] hover:bg-[#f9f9ff] transition shrink-0"
                      title="Add Item"
                    >
                          ...
                          {cat.items.length === 0 ? (
                            ...
                            cat.items.map((item) => (
                              ...
                              >
                                ...
                                <div className="opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity flex flex-wrap md:flex-nowrap items-center gap-0.5 md:gap-1 shrink-0 w-[60px] md:w-auto justify-end">
                                  <button
                                    onClick={(e) => { e.stopPropagation(); toggleItemAvailability(item.id, item.isAvailable); }}
                                    className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center text-[#74777d] hover:bg-white border border-transparent hover:border-[#e2e8f0] rounded transition shadow-sm"
                                  ...
                                  <button
                                    ...
                                    className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center text-[#74777d] hover:text-red-600 hover:bg-red-50 hover:border-red-100 border border-transparent rounded transition shadow-sm"
                                  >
                                    <Trash size={14} />
                                  </button>
                                </div>
